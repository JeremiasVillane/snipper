...
export async function generateMetadata({
  ...
  let redirectionURL = shortLink.originalUrl;
  ...
  if (
    ...
    redirectionURL = shortLink.expirationUrl;
  }

  const pageTitle = `Redirecting to ${redirectionURL}...`;
  const pageDescription = `You are being redirected via ${appName}.`;

  const ogTitle = shortLink.customOgTitle || `Link to ${redirectionURL}`;
  const ogDescription =
    shortLink.customOgDescription ||
    `Short link generated by ${appName} leading to ${redirectionURL}`;

  let ogImageUrl: string | undefined = undefined;
  let ogImageWidth = 1200; // Default OG width
  let ogImageHeight = 630; // Default OG height
  let ogImageAlt = `Preview of ${redirectionURL}`;
...
export default async function ShortCodePage({
  ...
  if (
    ...
    redirect(shortLink.expirationUrl);
  }
